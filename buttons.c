...
static char XRNrcsid[] = "$Header: /d/src/cvsroot/xrn/buttons.c,v 1.13 1994-10-11 14:51:59 jik Exp $";
#endif
    ...
    int mode;
{
	...
	for (i = 0; i < sizeof(Translations) / sizeof(struct _translations); i++) {
		...
		XtOverrideTranslations(Translations[i].widget, table);
            ...
            XtSetArg(args[0], XtNtranslations, (XtPointer) &translations);
            XtGetValues(Translations[i].widget, args, XtNumber(args));
            Translations[i].tables[mode] = translations;
    ...
    Cardinal *count;
{
    ...
    switch (status) {
    case NOMATCH:
	mesgPane(XRN_SERIOUS, FIRST_UNAVAIL_MSG, artNum);
	/* fall through */
