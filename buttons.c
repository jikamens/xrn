...
static char XRNrcsid[] = "$Header: /d/src/cvsroot/xrn/buttons.c,v 1.24 1994-11-13 20:41:44 jik Exp $";
#endif
...
static ButtonList NgButtonList[] = {
   ...
   "Scroll the newsgroups list forward"},
    {ngScrollBackArgs, XtNumber(ngScrollBackArgs),
   "Scroll the newsgroups list backward"},
#endif /* lint */
};
...
static ButtonList ArtButtonList[] = {
   ...
   "Mark all articles (up to the middle-button selected article) in the group as read"},
    ...
    int infoLine;
{
	...
	while ((token = strtok(ptr, ", \t\n")) != NIL(char)) {
	    ...
	    if (j == *size) {
		mesgPane(XRN_SERIOUS, BAD_BUTTON_NAME_MSG, token);
	    }
	    ptr = NIL(char);
	}
	*size = i;
    ...
    long *artNum;
{
	...
	*artNum = atol(&SubjectString[ArtPosition+2]);
	...
	while (getArticle(filename, question) != XRN_OKAY) {
	    mesgPane(XRN_SERIOUS | append, ART_NOT_AVAIL_MSG, *artNum);
	    append = XRN_APPEND;
#ifndef MOTIF
...
#ifdef WATCH
	    else xmSetIconAndName(ReadIcon);
#endif
	}
    }
#ifndef MOTIF
...
void catchUpNG()
{
	...
	while (left <= right) {
		...
		catchUp();
		 ...
		 mesgPane(XRN_SERIOUS, NO_SUCH_NG_DELETED_MSG, name);
	    }
	    else {
		 mesgPane(XRN_SERIOUS, UNKNOWN_ENTER_NG_RESPONSE_MSG, ret,
			  "catchUpNG");
	    }
...
void fedUpART()
{
    ...
    if (NewsgroupDisplayMode) {
        artQuitFunction(NULL, NULL, NULL, NULL);
    } else {
    ...
    Cardinal *count;
{
    static struct DialogArg args[] = {
        {"no",               generalHandler, (XtPointer) XRN_NO},
	{"yes",              generalHandler, (XtPointer) XRN_YES},
    };
    ...
    Cardinal *count;
{
	...
	if (NewsgroupDisplayMode && (status == ENTER_UNREAD)) {
	    mesgPane(XRN_INFO, DISPLAYING_LAST_UNREAD_MSG, name);
	    status = GOOD_GROUP;
	}
	if (status == GOOD_GROUP) {
	    ...
	    switchToArticleMode();
	     ...
	     mesgPane(XRN_INFO, PROBABLY_KILLED_MSG, name);
	     updateNewsgroupMode();
	     return;
	}
	    ...
	    mesgPane(XRN_SERIOUS, NewsgroupDisplayMode ? NO_ARTICLES_MSG :
		     PROBABLY_EXPIRED_MSG, name);
	    updateNewsgroupMode();
	    return;
	}
	else {
	    mesgPane(XRN_SERIOUS, UNKNOWN_ENTER_NG_RESPONSE_MSG, status,
		     "ngReadFunction");
	    return;
	}
    } else {
...
{
    static struct DialogArg args[] = {
	{"abort",            subscribeHandler, (XtPointer) XRNsub_ABORT},
	{"last group",       subscribeHandler, (XtPointer) XRNsub_LASTGROUP},
	{"first",            subscribeHandler, (XtPointer) XRNsub_FIRST},
	{"last",             subscribeHandler, (XtPointer) XRNsub_LAST},
	{"cursor position",  subscribeHandler, (XtPointer) XRNsub_CURRENT},
    };
    ...
    XtPointer call_data;
{
	...
	if (name[0] == '\0') {
	    mesgPane(XRN_INFO, NO_NG_SPECIFIED_MSG);
	}
	else {
	     ...
	     if (ret == XRN_NOUNREAD) {
		   ...
		   */
		  mesgPane(XRN_INFO, DISPLAYING_LAST_UNREAD_MSG,
			   CurrentGroup->name);
		  ret = GOOD_GROUP;
	     }
	     if (ret == GOOD_GROUP) {
		  ...
		  mesgPane(XRN_SERIOUS, NO_SUCH_NG_MSG, name);
	     }
	     else if (ret == XRN_NOMORE) {
		  mesgPane(XRN_SERIOUS, NO_ARTICLES_MSG, name);
	     }
	     else {
		  mesgPane(XRN_SERIOUS, UNKNOWN_ENTER_NG_RESPONSE_MSG, ret,
			   "gotoHandler");
...
{
    static struct DialogArg args[] = {
	{"abort",           gotoHandler, (XtPointer) XRNgoto_ABORT},
	{"go to newsgroup", gotoHandler, (XtPointer) XRNgoto_GOTO},
    };
    ...
    Cardinal *count;
{
	...
	ret = enterNewsgroup(LastGroup, ENTER_SETUP | ENTER_UNSUBBED |
			     ENTER_JUMPING);
	if (ret == XRN_NOUNREAD) {
	    mesgPane(XRN_INFO, DISPLAYING_LAST_UNREAD_MSG, LastGroup);
	    ret = GOOD_GROUP;
	}
	if (ret == GOOD_GROUP) {
	    switchToArticleMode();
	    ...
	    mesgPane(XRN_SERIOUS, NO_SUCH_NG_DELETED_MSG, LastGroup);
	}
	else if (ret == XRN_NOMORE) {
	    mesgPane(XRN_SERIOUS, NO_ARTICLES_MSG, LastGroup);
	}
	else {
	    mesgPane(XRN_SERIOUS, UNKNOWN_ENTER_NG_RESPONSE_MSG, ret,
		     "ngPrevGroupFunction");
	...
	mesgPane(XRN_INFO, NO_PREV_NG_MSG);
    }
    ...
    Cardinal *count;
{
    ...
    if (First == Last) {
	mesgPane(XRN_INFO, NO_GROUPS_SELECTED_MSG);
	return;	
    }
	...
	mesgPane(XRN_SERIOUS, NG_NOT_MOVED_MSG);
	resetSelection();
	return;
    }
    ...
    Cardinal *count;
{
	...
	if (ret == BAD_GROUP) {
	    mesgPane(XRN_SERIOUS | append, NO_SUCH_NG_DELETED_MSG, name);
	    mesgPane(XRN_SERIOUS | XRN_SAME_LINE, SKIPPING_TO_NEXT_NG_MSG);
	    append = XRN_APPEND;
	    (void) strcpy(LastGroup, name);
	    continue;
	}
	else if (ret == XRN_NOMORE) {
	    mesgPane(XRN_INFO | append, PROBABLY_EXPIRED_MSG, name);
	    mesgPane(XRN_INFO | XRN_SAME_LINE, SKIPPING_TO_NEXT_NG_MSG);
	    append = XRN_APPEND;
	    (void) strcpy(LastGroup, name);
	    continue;
	}
	else if (ret == XRN_NOUNREAD) {
	    mesgPane(XRN_INFO | append, PROBABLY_KILLED_MSG, name);
	    mesgPane(XRN_INFO | XRN_SAME_LINE, SKIPPING_TO_NEXT_NG_MSG);
	    append = XRN_APPEND;
	    (void) strcpy(LastGroup, name);
	    continue;
	    ...
	    mesgPane(XRN_SERIOUS | append, UNKNOWN_ENTER_NG_RESPONSE_MSG, ret,
		     "artNextGroupFunction");
	    append |= XRN_APPEND;
	    continue;
	}
    ...
    Cardinal *count;
{
	...
	case ERROR:
		...
		if (last_bad + 1 == artNum) {
			...
			mesgPane(XRN_SERIOUS | append, ART_NOT_AVAIL_MSG,
				 first_bad);
		    }
		    else {
			mesgPane(XRN_SERIOUS | append, ARTS_NOT_AVAIL_MSG,
				 first_bad, last_bad);
		    }
		    append = XRN_APPEND;
    ...
    if (last_bad) {
	if (first_bad == last_bad) {
	    mesgPane(XRN_SERIOUS | append, ART_NOT_AVAIL_MSG, first_bad);
	}
	else {
	    mesgPane(XRN_SERIOUS | append, ARTS_NOT_AVAIL_MSG,
		     first_bad, last_bad);
    ...
    XtPointer call_data;
{
	...
	mesgPane(XRN_INFO, NO_ART_NUM_MSG);
	PopDownDialog(GotoArticleBox);
	GotoArticleBox = 0;
	unbusyCursor();
	inCommand = 0;
    ...
    if (artNum == 0) {
	mesgPane(XRN_SERIOUS, BAD_ART_NUM_MSG, numberstr);
	PopDownDialog(GotoArticleBox);
	GotoArticleBox = 0;
	unbusyCursor();
	inCommand = 0;
...
{
    static struct DialogArg args[] = {
	{"abort",   gotoArticleHandler, (XtPointer) XRNgotoArticle_ABORT},
	{"doit", gotoArticleHandler, (XtPointer) XRNgotoArticle_DOIT},
    };
    ...
    XtPointer call_data;
{
	...
	if (LastRegexp == NIL(char)) {
	    mesgPane(XRN_INFO, NO_PREV_REGEXP_MSG);
	    CLEANUP;
	    return;	   
	}
	regexpr = LastRegexp;
...
{
    static struct DialogArg args[] = {
	{"abort",   subSearchHandler, (XtPointer) XRNsubSearch_ABORT},
	{"forward", subSearchHandler, (XtPointer) XRNsubSearch_FORWARD},
	{"back",    subSearchHandler, (XtPointer) XRNsubSearch_BACK},
    };
    ...
    Cardinal *count;
{
    ...
    if (PrevArticle == 0) {
	mesgPane(XRN_INFO, NO_PREV_ART_MSG);
	return;
    }
    ArtPosition = 0;
    ...
    if (getArticle(&filename, &question) != XRN_OKAY) {
	mesgPane(XRN_SERIOUS, ART_NOT_AVAIL_MSG, PrevArticle);
#ifndef MOTIF
...
{
    static struct DialogArg args[] = {
	{"abort", addHandler, (XtPointer) XRNadd_ABORT},
	{"add",   addHandler, (XtPointer) XRNadd_ADD},
    };
...
{
    static struct DialogArg args[] = {
	{"abort", saveHandler, (XtPointer) XRNsave_ABORT},
	{"save",  saveHandler, (XtPointer) XRNsave_SAVE},
    };
    ...
    Cardinal *count;
{
	...
	while (gbeg <= right) {
	    ...
	    if (ret == BAD_GROUP) {
		mesgPane(XRN_SERIOUS, NO_SUCH_NG_DELETED_MSG, newGroup);
		return;
	    }
	    else if (ret != GOOD_GROUP) {
		mesgPane(XRN_SERIOUS, UNKNOWN_ENTER_NG_RESPONSE_MSG, ret,
...
{
    static struct DialogArg args[] = {
	{"abort",     allHandler, (XtPointer) XRNadd_ABORT},
	{"subscribe", allHandler, (XtPointer) XRNadd_ADD},
    };
    ...
    Cardinal *count;
{
    ...
    ret = enterNewsgroup(newGroup, ENTER_SETUP | ENTER_UNSUBBED |
			 ENTER_JUMPING);
    if (ret == XRN_NOUNREAD) {
	mesgPane(XRN_INFO, DISPLAYING_LAST_UNREAD_MSG, newGroup);
	ret = GOOD_GROUP;
    }
	...
	mesgPane(XRN_SERIOUS, NO_ARTICLES_MSG, newGroup);
    }
    else {
	mesgPane(XRN_SERIOUS, UNKNOWN_ENTER_NG_RESPONSE_MSG, ret,
		 "allGotoFunction");
