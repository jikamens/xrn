...
static char XRNrcsid[] = "$Header: /d/src/cvsroot/xrn/buttons.c,v 1.20 1994-10-27 16:10:46 jik Exp $";
#endif
    ...
    Cardinal *count;
{
	...
	if (ret == BAD_GROUP) {
	    ...
	    append = XRN_APPEND;
	    mesgPane(XRN_SERIOUS | append, SKIPPING_TO_NEXT_NG_MSG);
	    (void) strcpy(LastGroup, name);
	    continue;
	}
	else if (ret == XRN_NOMORE) {
	    ...
	    append = XRN_APPEND;
	    mesgPane(XRN_INFO | append, SKIPPING_TO_NEXT_NG_MSG);
	    (void) strcpy(LastGroup, name);
	    continue;
	}
    ...
    Cardinal *count;
{
	...
	switch (status) {
	case NOMATCH:
	    /* moveToArticle doesn't display an error in this case */
