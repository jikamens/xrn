...
static char XRNrcsid[] = "$Header: /d/src/cvsroot/xrn/compose.c,v 1.27 1994-10-31 16:20:27 jik Exp $";
#endif
...
#include <maildef.h>
	...
	if (fieldExists("Subject:") == XawTextSearchError) {
		...
		mesgPane(XRN_SERIOUS | XRN_APPEND, EMPTY_ADDED_MSG);
		mesgPane(XRN_SERIOUS | XRN_APPEND, FILL_IN_RESEND_MSG);
	...
	if (fieldExists("Newsgroups:") == XawTextSearchError) {
		...
		mesgPane(XRN_SERIOUS | XRN_APPEND, DEFAULT_ADDED_MSG);
	    else {
		mesgPane(XRN_SERIOUS | XRN_APPEND, EMPTY_ADDED_MSG);
		mesgPane(XRN_SERIOUS | XRN_APPEND, FILL_IN_RESEND_MSG);
	    }
