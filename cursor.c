...
static char XRNrcsid[] = "$Id: cursor.c,v 1.26 1995-09-27 11:32:55 jik Exp $";
#endif
    ...
    long point;
{
    ...
    *groupName = XtRealloc(*groupName, len + 1);
    ...
    if (strncmp(ptr, UNSUBED_MSG, sizeof(UNSUBED_MSG)-1) == 0)
	*mode = UNSUBSCRIBE;
    else if (strncmp(ptr, IGNORED_MSG, sizeof(IGNORED_MSG)-1) == 0)
	*mode = IGNORE;
    else
	*mode = SUBSCRIBE;
