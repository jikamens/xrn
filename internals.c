...
static char XRNrcsid[] = "$Id: internals.c,v 1.71 1995-01-25 03:17:52 jik Exp $";
#endif
    ...
    Boolean unread_only;
{
	...
	for (i = newsgroup->last; i >= newsgroup->first; i--) {
	    long indx = INDEX(i);
	    if (! IS_UNAVAIL(articles[indx]) && articleIsAvailable(newsgroup, i)) {
		newsgroup->current = i;
		return;
	    }
	}
...
void prefetchNextArticle()
{
    ...
    if (FastServer && IS_UNFETCHED(articles[indx])) {
	/*
    ...
    art_num artNum;
{
    ...
    *start = '\0';
	...
	if (articles[indx].subject &&
	    ((mode == ALL) ||
	     (IS_UNREAD(articles[indx]) && !IS_UNAVAIL(articles[indx])))) {
    ...
    int flags;
{
	...
	if ((newsgroup->current > newsgroup->last) ||
	    IS_READ(newsgroup->articles[INDEX(newsgroup->current)]))
	    ret = XRN_NOUNREAD;
	else
	    ret = GOOD_GROUP;
