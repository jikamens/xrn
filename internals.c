...
static char XRNrcsid[] = "$Id: internals.c,v 1.206 1997-06-30 02:52:27 jik Exp $";
#endif
...
#endif
	if (! (art->file = utGetarticle(newsgroup, article, 0,
    ...
    XtPointer pClient;
{
	...
	if (IS_UNFETCHED(art)) {
	    ...
	    if ((art->file = utGetarticle(newsgroup, newsgroup->current, 0,
...
void prefetchNextArticle()
{
	...
	if (IS_UNFETCHED(art) && 
	    (art->file = utGetarticle(newsgroup, artnum, 0,
				      NORMAL_HEADER, NOT_ROTATED,
				      NOT_XLATED))) {
	    SET_FETCHED(art);
	    SET_STRIPPED_HEADERS(art);
