...
static char XRNrcsid[] = "$Id: internals.c,v 1.87 1995-02-08 01:44:28 jik Exp $";
#endif
    ...
    int mode;	/* 0 for unread groups, 1 for all subscribed to groups */
{
    ...
    char *string, **ar;
    int newsgroup_length = TextGetColumns(Text) - NEWS_GROUP_LINE_CHARS;
    ...
    int sorted;
{
    ...
    if (! status_width)
	status_width = MAX(utStrlen(SUBED_MSG), utStrlen(UNSUBED_MSG));

    newsgroup_width = TextGetColumns(ArticleText) - status_width - 1
    ...
    art_num artNum;
{
    ...
    if (artNum <= 0) {
	lineLength = TextGetColumns(Text);
