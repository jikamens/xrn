...
static char XRNrcsid[] = "$Id: internals.c,v 1.37 1994-11-29 22:17:16 jik Exp $";
#endif
...
#include <ctype.h>
#endif /* VMS */
#if defined(aiws) || defined(_POSIX_SOURCE)
#include <unistd.h>
#endif /* aiws || _POSIX_SOURCE */
#include "avl.h"
#include "news.h"
    ...
    int type;
{
	...
	if ((*iptr == '/')  ||
	    (*iptr == '\\') ||
	    (*iptr == '(')  ||
	    /* On some systems, it's an error for there to be a ')'
	       with no matching '(' before it. */
	    (*iptr == ')')  ||
	    (*iptr == '[')  ||
	    ...
	    (*iptr == '\'') ||
	    (*iptr == '\"')) {
	    /* XXX Why replace with '.' instead of quoting with
	       backslash?  I don't know the answer to this, but on the
	       off chance that there's a reason, I'm not going to
	       change it. -- jik 10/12/94 */
	    *iptr = '.';
	}
	*optr++ = *iptr++;
    }
    ...
    FILE *fp;
{
    ...
    (void) sprintf(dummy, "Processing KILL file for newsgroup `%s'...",
		   newsgroup->name);
    info(dummy);
...
#define printcount(c,cmd,m) \
	...
	mesgPane(XRN_INFO | append, m, c, ((c==1) ? "" : "s"), \
		 newsgroup->name); \
	append = XRN_APPEND; \
    }

    printcount(kcount, 'j', "killed %d article%s in %s");
    printcount(mcount, 'm', "marked %d article%s unread in %s");
    printcount(scount, 's', "saved %d article%s in %s");
    printcount(ucount, '\0', "matched %d article%s with unknown option in %s");

#undef printcount

    (void) sprintf(dummy, "Processing KILL file for newsgroup `%s'... done.",
		   newsgroup->name);
    info(dummy);
 ...
 */
static void setUpGroup _ARGUMENTS((struct newsgroup *, int, /* Boolean */ int));

static void setUpGroup(newsgroup, stage, update_last)
    struct newsgroup *newsgroup;
    int stage;
    Boolean update_last;
{
    ...
    struct newsgroup *newsgroup;
{
    ...
    if (found) {
	if (unread <= 0) {
	    (void) sprintf(dummy, "Art. %ld in %s (Next: %s, %d articles)",
			   newsgroup->current, newsgroup->name,
			   nextnewsgroup->name, unreadArticleCount(nextnewsgroup));
	} else {
	    (void) sprintf(dummy, "Art. %ld in %s (%ld left) (Next: %s, %d articles)",
			   newsgroup->current, newsgroup->name, unread,
			   nextnewsgroup->name, unreadArticleCount(nextnewsgroup));
	}
    } else {
	if (unread <= 0) {
	    (void) sprintf(dummy, "Art. %ld in %s",
			   newsgroup->current, newsgroup->name);
	} else {
	    (void) sprintf(dummy, "Art. %ld in %s (%ld left)",
			   newsgroup->current, newsgroup->name, unread);
	}
    }
    return dummy;
    ...
    char **question;
{
    ...
    if (ret != XRN_OKAY) {
	mesgPane(XRN_SERIOUS, ART_NOT_AVAIL_MSG, newsgroup->current);
    }
    return ret;
}
...
#define doFindUnread(a) \
	...
	if (a == XRN_NOMORE) { \
	    ...
	    mesgPane(XRN_SERIOUS, UNKNOWN_FIND_UNRD_RESPONSE_MSG, a, \
		     "enterNewsgroup"); \
	    return a; \
	}
...
#ifdef SYSV_REGEX
	if (! (l[n] = regcmp(q, 0))) {
	    mesgPane(XRN_SERIOUS, UNKNOWN_LIST_REGEXP_ERROR_MSG, list_name, q);
	    continue;
	}
#else
	l[n] = q;
	if ((q = re_comp(l[n]))) {
	    mesgPane(XRN_SERIOUS, KNOWN_LIST_REGEXP_ERROR_MSG, list_name, l[n], q);
	    continue;
	}
#endif
	n++;
