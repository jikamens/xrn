...
ButtonList ArtButtonList[] = {
    ...
    {"artKillSession",		artKillSessionCallbacks,	ARTKILLSESSION_EXSTR},
    {"artKillLocal",		artKillLocalCallbacks,		ARTKILLLOCAL_EXSTR},
    {"artKillGlobal",		artKillGlobalCallbacks,		ARTKILLGLOBAL_EXSTR},
...
#ifdef POSIX_REGEX
	reRet = regcomp(&reStruct, subject, REG_NOSUB);
	assert (! reRet);
#else /* ! POSIX_REGEX */
# ifdef SYSV_REGEX
...
#ifdef POSIX_REGEX
			(! regexec(&reStruct, cursubject, 0, 0, 0))
#else
# ifdef SYSV_REGEX
...
# endif
	...
	infoNow(author ? ERROR_AUTHOR_KILL_MSG : ERROR_SUB_KILL_MSG);
    }
    ...
    Cardinal *count;
{
	...
	killItem(newsgroup, SubjectKilled, "Subject", KILL_LOCAL);
    return;
}
    ...
    Cardinal *count;
{
    ...
    if ((SubjectKilled = _artKillSession(widget, False)))
	killItem(newsgroup, SubjectKilled, "Subject", KILL_GLOBAL);
    return;
}
    ...
    Cardinal *count;
{
    ...
    finished = ((! abortP()) && (fillUpArray(firstArticle(), 0, True) != ABORT));
...
#ifdef POSIX_REGEX
	regfree(&reStruct);
	if ((reRet = regcomp(&reStruct, expr, REG_NOSUB)))
#else
# ifdef SYSV_REGEX
	FREE(reRet);
