...
static char XRNrcsid[] = "$Header: /d/src/cvsroot/xrn/newsrcfile.c,v 1.7 1994-11-13 19:02:32 jik Exp $";
#endif
    ...
    char *save;  /* name of file to save to */
{
	...
	mesgPane(XRN_SERIOUS, CANT_OPEN_NEWSRC_COPYING_MSG, old,
		 errmsg(errno));
	return FATAL;
    }

    if ((newFile = utNameExpand(save)) == NIL(char)) {
	mesgPane(XRN_SERIOUS, CANT_EXPAND_NEWSRC_SAVE_MSG, save);
	return FATAL;
    }

    if (STREQ(newFile, "")) {
	 mesgPane(XRN_SERIOUS, EMPTY_NEWSRC_SAVE_NAME_MSG);
	 return FATAL;
    }
	...
	mesgPane(XRN_SERIOUS, CANT_OPEN_NEWSRC_SAVE_MSG, save, errmsg(errno));
	return FATAL;
    }

    while ((num_read = fread(buf, sizeof(char), BUFSIZ, orig))) {
	 if (! fwrite(buf, sizeof(char), num_read, new)) {
	     mesgPane(XRN_SERIOUS, NEWSRC_SAVE_FILE_WRITE_ERR_MSG, save,
		      errmsg(errno));
	      return FATAL;
	 }
    }
    ...
    char *savenewsrcfile;
{
	...
	mesgPane(XRN_SERIOUS, CANT_EXPAND_NEWSRC_MSG, newsrcfile);
	return FATAL;
    }
	...
	if (errno != ENOENT) {
	    mesgPane(XRN_SERIOUS, CANT_READ_NEWSRC_MSG, NewsrcFile,
		     errmsg(errno));
	    return FATAL;
	}
	mesgPane(XRN_INFO, CREATING_NEWSRC_MSG, NewsrcFile);
	if ((Newsrcfp = fopen(NewsrcFile, "w")) == NULL) {
	    mesgPane(XRN_SERIOUS, CANT_CREATE_NEWSRC_MSG, NewsrcFile,
		     errmsg(errno));
	    return FATAL;
	}
...
#ifdef NO_FCHMOD
	(void) chmod(NewsrcFile, fbuf.st_mode);
#endif
    }

    if (stat(NewsrcFile, &buf) == -1) {
	mesgPane(XRN_SERIOUS, CANT_STAT_NEWSRC_MSG, NewsrcFile,
		 errmsg(errno));
	return FATAL;
    }
    
    if (buf.st_size == 0) {
	mesgPane(XRN_SERIOUS, ZERO_LENGTH_NEWSRC_MSG, NewsrcFile);
	return FATAL;
    }

    if ((Newsrcfp = fopen(NewsrcFile, "r")) == NULL) {
	mesgPane(XRN_SERIOUS, CANT_OPEN_NEWSRC_MSG, NewsrcFile, errmsg(errno));
	return FATAL;
    }

    if (yyparse() != 0) {
	mesgPane(XRN_SERIOUS, CANT_PARSE_NEWSRC_MSG, NewsrcFile,
		 MaxGroupNumber +1);
	return FATAL;
    }
...
int updatenewsrc()
{
	...
	mesgPane(XRN_SERIOUS, CANT_OPEN_NEWSRC_TEMP_MSG, tempfile,
		 errmsg(errno));
	return(FATAL);
    }
	...
	mesgPane(XRN_SERIOUS, CANT_OPEN_NEWSRC_WRITING_MSG, NewsrcFile,
		 errmsg(errno));
	return(FATAL);
    }
#endif /* VMS */
	...
	mesgPane(XRN_SERIOUS, ERROR_UNLINKING_NEWSRC_MSG, NewsrcFile,
		 errmsg(errno));
	return(FATAL);
    }
#endif /* ISC_TCP */
	...
	mesgPane(XRN_SERIOUS, ERROR_RENAMING_NEWSRC_MSG, tempfile, NewsrcFile,
		 errmsg(errno));
	return(FATAL);
    }
#endif /* VMS */
