...
static char XRNrcsid[] = "$Id: save.c,v 1.17 1994-12-16 19:20:21 jik Exp $";
#endif
...
int saveArticleByNumber(filename, art)
    ...
    art_num art;
{
    return saveArticle(filename, CurrentGroup, art, False);
}

int saveArticle(filename, newsgroup, art, temporaryp)
    char *filename;
    ...
    Boolean temporaryp;
{
	...
	FREE(artfile);
	(void) sprintf(error_buffer, "MH refile to folder %s %s", filename,
		       status ? "done" : "failed");
	infoNow(error_buffer);
	if (status) {
	    SET_SAVED(newsgroup->articles[INDEX(art)]);
	...
	(void) sprintf(error_buffer, "RMAIL refile to folder %s %s",
			filename+1, status ? "done" : "failed");
	infoNow(error_buffer);
	(void) unlink(artfile);
	FREE(artfile);
	return(status);
 ...
 *
 */
int saveCurrentArticle(filename, temporaryp)
    char *filename;
    Boolean temporaryp;
{
