...
static void updateSubjectWidget(string, left, right, update_top, preserve_top)
    String string;
    long left, right;
    Boolean update_top, preserve_top;
{
    Boolean disable = False;
    long top;

    if (preserve_top)
	top = TextGetTopPosition(Text);
...
int switchToArticleMode()
{
	...
	setButtonSensitive("artHeader", False);
    setButtonSensitive("artLast", False);
    setButtonSensitive("artContinue", False);
    setButtonSensitive("artPost", PostingAllowed);
    setButtonSensitive("artFollowup", PostingAllowed);
    setButtonSensitive("artFollowupAndReply", PostingAllowed);
    setButtonSensitive("artListOld", True);
    ...
    char *filename, *question;
{
	...
	if (canCancelArticle())
	    setButtonSensitive("artCancel", True);
	else
	    setButtonSensitive("artCancel", False);
    ...
    char *file, *ques;
{
	...
	setButtonSensitive("artLast", True);
...
{
    TextDisableRedisplay(Text);
    TextSetInsertionPoint(Text, TextGetLength(Text));
    _catchUpPartART(True);
    TextEnableRedisplay(Text);
}
    ...
    Boolean author;
{
	...
	subject = stringToRegexp(subject);

#ifdef SYSV_REGEX
	assert(reRet = regcmp(subject, 0));
#else
	assert(! re_comp(subject));
#endif
	...
	while (SubjectString[left] != '\0') {
		...
		if (! cursubject) {
		    mesgPane(XRN_SERIOUS, 0, ART_NOT_AVAIL_MSG, artNum);
		    TextRemoveLine(Text, left);
		    removeLine(SubjectString, &left);
		}
		else if
#ifdef SYSV_REGEX
	                (regex(reRet, cursubject))
#else
	                (re_exec(cursubject))
#endif
		    {
    ...
    Cardinal *count;
{
    ...
    AuthorKilled = _artKillSession(widget, True);
}
    ...
    Cardinal *count;
{
    ...
    setButtonSensitive("artListOld", False);
    ...
    XtPointer call_data;
{
	...
	if (*regexpr == 0) {
	    ...
	    setButtonSensitive("artContinue", True);
	}
    }
