...
static char XRNrcsid[] = "$Id: internals.c,v 1.53 1994-12-09 18:28:06 jik Exp $";
#endif
    ...
    Boolean prefetching;
{
	...
	killArticles(newsgroup, fp, prefetching ? chunk_size : 0);
	(void) fclose(fp);
    }
    ...
    Boolean update_last;
{
...
#ifdef DEBUG
    fprintf(stderr, "setUpGroupIncremental(%s, %d, %d)\n", newsgroup->name,
	    stage, update_last);
#endif
	...
	if (! EMPTY_GROUP(newsgroup)) {
		...
		finished = checkKillFiles(newsgroup, stage ? True : False);
	}
    }
    ...
    fprintf(stderr, "setUpGroupIncremental(%s, %d, %d) = %d\n",
	    newsgroup->name, stage, update_last, finished);
#endif
    return finished;
}
...
static void setUpGroup(newsgroup, update_last)
    struct newsgroup *newsgroup;
    Boolean update_last;
{
#ifdef DEBUG
    fprintf(stderr, "setUpGroup(%s, %d)\n", newsgroup->name, update_last);
#endif
    (void) setUpGroupIncremental(newsgroup, 0, update_last);
#ifdef DEBUG
    fprintf(stderr, "setUpGroup(%s, %d) done\n", newsgroup->name, update_last);
#endif
}
