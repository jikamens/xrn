...
static char XRNrcsid[] = "$Id: server.c,v 1.58 1995-02-21 03:09:21 jik Exp $";
#endif
...
Boolean ServerDown = False, PostingAllowed = True;
    ...
    int header, rotation, xlation;
{
	...
	if (*msg != '\0') {
	    ...
	    if (*msg == '\014') {
		int i, lines;
		lines = TextGetLines(ArticleText);
	...
	return getarticle(newsgroup, artnumber, retposition, header,
			  rotation, xlation);
    return(NIL(char));
}
...
void getactive()
{
    char command[MESSAGE_SIZE], message[MESSAGE_SIZE], *group;
