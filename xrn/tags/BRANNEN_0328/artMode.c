    ...
    long *artNum;
{
	...
	*artNum = atol(&SubjectString[ArtPosition + 2]);
	...
	if (getArticle(CurrentGroup, *artNum, filename, question) != XRN_OKAY) {
    ...
    art_num *artNum;
{
	...
	if (ArtPosition != 0) {
	    ...
	    *artNum = atol(&SubjectString[ArtPosition+2]);
		...
		if (getArticle(CurrentGroup, *artNum, filename, question)
		    != XRN_OKAY) {
		  art_unavailable:
...
int switchToArticleMode()
{
    ...
    FREE(SubjectString);
    FirstListedArticle = CurrentGroup->current;
    ...
    art = artStructGet(CurrentGroup, FirstListedArticle, True);
    SET_LISTED(art);
    artStructSet(CurrentGroup, &art);

    for (i = FirstListedArticle + 1; i <= CurrentGroup->last; i++) {
...
static Boolean do_field_kill _ARGUMENTS((char *, int, _fixfunction, int, XEvent *,
					 String *, Cardinal *, Boolean));
...
void artKillSubjectFunction(widget, event, string, count)
    ...
    Cardinal *count;
{
  (void) do_field_kill("Subject", SUBJ_OFFSET, subjectStrip, SUBJ_OFFSET,
...
void artKillAuthorFunction(widget, event, string, count)
    ...
    Cardinal *count;
{
  (void) do_field_kill("From", FROM_OFFSET, 0, FROM_OFFSET,
		       event, string, count, True);
}
     ...
     Cardinal *count;
{
  ...
  if (! do_field_kill("References", REFS_OFFSET, getFirstReference, REFS_OFFSET,
		      event, string, count, False))
    (void) do_field_kill("References", ID_OFFSET, getFirstReference, REFS_OFFSET,
			 event, string, count, True);
}
    ...
    Cardinal *count;
{
    ...
    finished = ((! abortP()) &&
		(fillUpArray(CurrentGroup, CurrentGroup->first, 0,
			     True, False) != ABORT));
    ...
    if (finished) {
      ...
      for (i = FirstListedArticle; i <= CurrentGroup->last; i++) {
	art = artStructGet(CurrentGroup, i, False);
	if (IS_AVAIL(art) && !IS_LISTED(art)) {
      ...
      for (i = FirstListedArticle - 1; i >= CurrentGroup->first; i--) {
	art = artStructGet(CurrentGroup, i, False);
	if (IS_AVAIL(art)) {
	  copy = *art;
	  SET_LISTED(&copy);
	  artStructReplace(CurrentGroup, &art, &copy, i);
	  FirstListedArticle = i;
	}
      }
    ...
    XtPointer call_data;
{
	...
	if ((ArtPosition = findArticle(SubjectString, artNum, True)) == -1) {
	  ...
	  if (artNum < FirstListedArticle) {
	    for (i = FirstListedArticle - 1; i >= artNum; i--) {
	      struct article *art = artStructGet(CurrentGroup, i, False);
	      if (IS_AVAIL(art)) {
		struct article copy = *art;
		SET_LISTED(&copy);
		artStructReplace(CurrentGroup, &art, &copy, i);
    ...
    Cardinal *count;
{
    ...
    if (getArticle(CurrentGroup, PrevArticle, &filename, &question) != XRN_OKAY) {
