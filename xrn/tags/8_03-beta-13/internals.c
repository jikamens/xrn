...
static char XRNrcsid[] = "$Id: internals.c,v 1.177 1996-07-29 16:20:38 jik Exp $";
#endif
     ...
     Boolean unread_only;
{
  ...
  for (i = first; i <= newsgroup->last; i++) {
    ...
    if (art->parent) {
      ...
      if ((art->parent < first) ||
	  (unread_only && (IS_READ(parent) || IS_READ(art)))) {
	copy = *parent;
    ...
    int status;
{
	...
	if ((ConfirmationBox(TopLevel, err_buf, "yes", "no", True) ==
	     XRN_CB_ABORT) || (! verifyGroup(afterGroup, &ng, False))) {
