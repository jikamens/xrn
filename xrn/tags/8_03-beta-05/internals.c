...
static char XRNrcsid[] = "$Id: internals.c,v 1.161 1996-06-07 14:21:48 jik Exp $";
#endif
...
#define PREFETCH_LAST_HEADERS_STAGE	PREFETCH_DATE_STAGE
...
static Boolean fetchHeadersIncremental(newsgroup, stage, first, last,
				       unread_only, fetch_newsgroups,
				       fetch_dates)
     struct newsgroup *newsgroup;
     int *stage;
     art_num first, last;
     Boolean unread_only, fetch_newsgroups, fetch_dates;
{
		...
		((*stage >= PREFETCH_START_HEADERS_STAGE) &&
		 (*stage <= PREFETCH_LAST_HEADERS_STAGE)))
	      finished = fetchHeadersIncremental(newsgroup, stage,
						 my_first, my_last,
						 unread_only,
						 newsgroup->fetch_newsgroups &&
						 kill_files,
						 newsgroup->fetch_dates);
     ...
     int line_length;
{
  ...
  if (art) {
    (void) subjectIndexLine(line_length, buffer, newsgroup, art);
    return buffer;
  }
    ...
    Boolean check_abort;
{
    ...
    for (i = PREFETCH_START_HEADERS_STAGE; i <= PREFETCH_LAST_HEADERS_STAGE; ) {
      if (fetchHeadersIncremental(newsgroup, &i, art, last,
				  False, False, newsgroup->fetch_dates))
	i++;
