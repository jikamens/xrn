...
static char XRNrcsid[] = "$Id: internals.c,v 1.43 1994-12-05 18:42:42 jik Exp $";
#endif
    ...
    FILE *fp;
{
	...
	mesg_name = "killArticles (parsing KILL entry)";
	...
	*pptr = '\0';
	...
	mesg_name = "killArticles (killing)";
		...
		if ((subject && re_exec(subj)) ||
			...
			case 'j':
			    ...
			    if (index(app_resources.verboseKill, 'j')) {
			        mesgPane(XRN_INFO, mesg_name, "killed - %s",
					 subject);
			    }
			    kcount++;
			    break;
			...
			case 's':
				...
				mesgPane(XRN_INFO, mesg_name, "saved - %s",
					 subject);
			    }
			    scount++;
			    break;
    ...
    mesg_name = "killArticles (summarizing)";

#define printcount(c,cmd,m) \
    if (c && ((! cmd) || index(app_resources.verboseKill, cmd))) { \
	mesgPane(XRN_INFO, mesg_name, m, c, ((c==1) ? "" : "s"), \
		 newsgroup->name); \
    }
