...
static char XRNrcsid[] = "$Id: internals.c,v 1.156 1996-06-06 18:02:02 jik Exp $";
#endif
...
static Boolean setCurrentArticle(newsgroup, unread_only, finished, max)
    struct newsgroup *newsgroup;
    Boolean unread_only;
    Boolean *finished;
    int max;
{
	...
	if (IS_UNREAD(art) && IS_AVAIL(art)) {
	  if (articleIsAvailable(newsgroup, i)) {
	    if (finished)
	      *finished = True;
	    newsgroup->current = i;
	    goto done;
 ...
 */
void killItem(newsgroup, item, field, type)
    struct newsgroup *newsgroup;
    char *item;
    char *field;
    int type;
...
/*
  Parse a KILL-file entry.  Return a pointer into a static entry
  structure if successful, or null if unsuccessful (i.e., the line
  should be ignored).  If there's an error, this routine will display
  it.

  The kill_entry structure contains a pointer to the compiled regexp
  (if necessary), a set of flags indicating which fields to check
  (right now, only subject and author are supported), and a set of
  flags indicating the action (junk the article, mark it read, or save
  it).

  Lines beginning with "THRU", lines beginning with "&", lines
  beginning with "#", and blank lines (or lines containing only
  ...
  and newsgroup->fetch_newsgroups is false, it will set
  newsgroup->fetch_newsgroups to true.  The caller should detect this
  and react appropriately. Ditto for fetch_dates.
  */
static kill_entry *parse_kill_entry(newsgroup, str)
     struct newsgroup *newsgroup;
     char *str;
{
  static kill_entry entry = {
#ifdef POSIX_REGEX
    {0},
#endif
    0};
...
# endif /* ! SYSV_REGEX */
  ...
  /*
    Ignore blank lines and whitespace at the beginning of the line.
    */
  ...
  if (index(app_resources.verboseKill, 'l'))
    mesgPane(XRN_INFO, mesg_name, KILL_LINE_MSG, str, newsgroup->name);

  if (*ptr != '/') {
    mesgPane(XRN_SERIOUS, mesg_name, MALFORMED_KILL_ENTRY_MSG, str,
	     newsgroup->name, ERROR_REGEX_NOSLASH_START_MSG);
    return 0;
  }
  ...
  /*
    ...
    */
  strncpy(pattern + 1, ptr, MIN(ptr2 - ptr, BUFFER_SIZE - 1));
  pattern[MIN(ptr2 - ptr + 1, BUFFER_SIZE - 1)] = '\0';
  ...
  if (*++ptr2 == 'h') {
    /*
      For now, the only header lines we're supporting are "From",
      "Subject" and "Newsgroups". XXX
      */
    if (! strncmp(ptr, "^Subject:", 9)) {
      ptr += 9;
      entry.check_flags = KILL_SUBJECT;
    }
    else if (! strncmp(ptr, "^From:", 6)) {
      ptr += 6;
      entry.check_flags = KILL_AUTHOR;
    }
    else if (! strncmp(ptr, "^Newsgroups:", 12)) {
      ptr += 12;
      entry.check_flags = KILL_NEWSGROUPS;
    }
    else if (! strncmp(ptr, "^Date:", 6)) {
      ptr += 6;
      entry.check_flags = KILL_DATE;
    }

    if (entry.check_flags) {
      while (*ptr && isspace(*ptr))
	ptr++;
      *--ptr = '^';
    }
    else
      entry.check_flags = KILL_SUBJECT | KILL_AUTHOR | KILL_NEWSGROUPS |
			     KILL_DATE;
  }
  else
    /*
      ...
      */
    entry.check_flags = KILL_SUBJECT | KILL_AUTHOR;

  entry.action_flags = 0;
  ...
  switch (*++ptr2) {
  case 'j':
    entry.action_flags = KILL_JUNK;
    break;
  case 'm':
    entry.action_flags = KILL_MARK;
    break;
  case 's':
    entry.action_flags = KILL_SAVE;
    break;
  default:
    mesgPane(XRN_SERIOUS, mesg_name, MALFORMED_KILL_ENTRY_MSG, str,
...
#ifdef POSIX_REGEX
  if (entry.inited) {
    regfree(&entry.reStruct);
    entry.inited = 0;
  }
  if ((reRet = regcomp(&entry.reStruct, ptr, REG_NOSUB))) {
    regerror(reRet, &entry.reStruct, error_buffer, sizeof(error_buffer));
...
#else
# ifdef SYSV_REGEX
  XtFree(entry.reStruct);
  if (! (entry.reStruct = regcmp(ptr, NULL))) {
    mesgPane(XRN_SERIOUS, mesg_name, UNKNOWN_KILL_REGEXP_ERROR_MSG, str);
    return 0;
  }
# else
...
# endif /* SYSV_REGEX */
  ...
  if (entry.check_flags & KILL_NEWSGROUPS)
    newsgroup->fetch_newsgroups = TRUE;
  if (entry.check_flags & KILL_DATE)
    newsgroup->fetch_dates = TRUE;
    ...
    int max;
{
    ...
    char *subject, *author, *newsgroups, *date;
    int scount = 0, kcount = 0, mcount = 0;
    int mesg_name;
    kill_entry *entry;
    unsigned char fetch_newsgroups = newsgroup->fetch_newsgroups;
    unsigned char fetch_dates = newsgroup->fetch_dates;
    ...
    while (((fp1 && fgets(string, sizeof(string), fp1)) || (fp1 = 0)) ||
      ...
      if (! (entry = parse_kill_entry(newsgroup, string)))
	continue;

      if (newsgroup->fetch_newsgroups && !fetch_newsgroups) {
	fetch_newsgroups = True;
	(void) getnewsgroupslist(newsgroup, start, last, True, 0);
      }
      if (newsgroup->fetch_dates && !fetch_dates) {
	fetch_dates = True;
	(void) getdatelist(newsgroup, start, last, True, 0);
      }
	...
	/* short cut */
	if (IS_KILLED(art) || IS_UNAVAIL(art) ||
	    ((entry->action_flags & KILL_JUNK) && IS_READ(art)) ||
	    ((entry->action_flags & KILL_MARK) && IS_UNREAD(art)))
	  continue;

	if (entry->check_flags & KILL_SUBJECT)
	  subject = art->subject;
	else
	  subject = 0;

	if (entry->check_flags & KILL_AUTHOR)
	  author = art->from;
	else
	  author = 0;
	...
	if (subject || author || newsgroups || date) {
	  struct article copy = *art;
	  unsigned char changed = FALSE;

	  if (
#ifdef POSIX_REGEX
	      (subject && (! regexec(&entry->reStruct, subject, 0, 0, 0))) ||
	      (author  && (! regexec(&entry->reStruct, author, 0, 0, 0))) ||
	      (date  && (! regexec(&entry->reStruct, date, 0, 0, 0))) ||
	      (newsgroups  && (! regexec(&entry->reStruct, newsgroups, 0, 0, 0)))
#else
# ifdef SYSV_REGEX
	      (subject && (regex(&entry->reStruct, subject) != NULL)) ||
	      (author  && (regex(&entry->reStruct, author) != NULL)) ||
	      (date  && (regex(&entry->reStruct, date) != NULL)) ||
	      (newsgroups  && (regex(&entry->reStruct, newsgroups) != NULL))
# else
	      (subject && re_exec(subject)) ||
	      (author  && re_exec(author)) ||
	      (date  && re_exec(date)) ||
	      (newsgroups  && re_exec(author))
# endif
#endif
	      ) {
	    switch (entry->action_flags) {
	    case KILL_JUNK:
	      SET_READ(&copy);
	    ...
	    default:
	      mesgPane(XRN_SERIOUS, mesg_name, UNKNOWN_FUNC_RESPONSE_MSG,
		       entry->action_flags, "parse_kill_entry(action_flags)",
		       "killArticles");
	      break;
	    }
	  }
...
#define PREFETCH_START_FETCH_STAGE	PREFETCH_SUBJECT_STAGE
...
#define PREFETCH_LAST_FETCH_STAGE	PREFETCH_DATE_STAGE
#define PREFETCH_KILL_STAGE		8
#define PREFETCH_LAST_STAGE		PREFETCH_KILL_STAGE
    ...
    art_num first, last;
{
	...
	if (! EMPTY_GROUP(newsgroup)) {
	    ...
	    if (stage &&
		(*stage >= PREFETCH_GETGROUP_STAGE) &&
		(*stage <= PREFETCH_LAST_FETCH_STAGE))
		start_time = time(0);
	    ...
	    if (! stage || *stage == PREFETCH_SUBJECT_STAGE)
	      finished = getsubjectlist(newsgroup, my_first, my_last,
					unread_only,
					(stage && ! FinishingPrefetch)
					? chunk_size : 0);
	    if (! stage || *stage == PREFETCH_AUTHOR_STAGE)
	      finished = getauthorlist(newsgroup, my_first, my_last,
				       unread_only,
				       (stage && ! FinishingPrefetch)
				       ? chunk_size : 0);
	    if (! stage || *stage == PREFETCH_LINES_STAGE)
	      finished = getlineslist(newsgroup, my_first, my_last,
				      unread_only,
				      (stage && ! FinishingPrefetch)
				      ? chunk_size : 0);
	    if (newsgroup->fetch_newsgroups &&
		(! stage || *stage == PREFETCH_NEWSGROUPS_STAGE) &&
		kill_files)
	      finished = getnewsgroupslist(newsgroup, my_first, my_last,
					   unread_only,
					   (stage && ! FinishingPrefetch)
					   ? chunk_size : 0);
	    if (newsgroup->fetch_dates &&
		(! stage || *stage == PREFETCH_DATE_STAGE))
	      finished = getdatelist(newsgroup, my_first, my_last,
				     unread_only,
				     (stage && ! FinishingPrefetch)
				     ? chunk_size : 0);
	    ...
	    if (! stage || *stage == PREFETCH_KILL_STAGE) {
		  ...
		  finished = checkKillFiles(newsgroup,
					    (stage && ! FinishingPrefetch)
					    ? True : False);
		  if (! finished && newsgroup->fetch_newsgroups &&
		      ! fetch_newsgroups)
		    if (! stage)
		      ...
		      *stage = PREFETCH_NEWSGROUPS_STAGE;
		      finished = False;
		    }

		  if (! finished && newsgroup->fetch_dates &&
		      ! fetch_dates)
		    if (! stage)
		      return setUpGroupIncremental(newsgroup, stage, update_last,
						   kill_files, unread_only,
		      ...
		      *stage = PREFETCH_DATE_STAGE;
		      finished = False;
		    }
		}
		if (finished) {
		  if (setCurrentArticle(newsgroup, unread_only, 0, 0)) {
		    if (! stage)
		      return setUpGroupIncremental(newsgroup, stage, update_last,
						   kill_files, unread_only,
						   first, last);
...
/*
  ...
  calls to it.
  */
int subjectIndexLine(line_length, out, newsgroup, artNum)
    int line_length;
    char *out;
    struct newsgroup *newsgroup;
    art_num artNum;
{
    ...
    (void) sprintf(out, "  %*ld %*.*s %*.*s %*.*s\n", num_width,
		   artNum, -subLength, subLength, subject, lineWidth,
...
char * getSubjects(line_length, mode, artNum)
    ...
    art_num artNum;
{
  return art_sort_doit(CurrentGroup, artNum, CurrentGroup->last,
		       mode != ALL, line_length);
}
    ...
    char *str;
{
    ...
    for (end = start; (end = index(end, '(')); end++)
    ...
    int line_length;
{
	...
	if (! art->subject) {
	  (void) fillUpArray(MAX(newsgroup->first, NextPreviousArticle - SUBJECTS),
...
int fillUpArray(art, last, check_abort)
    art_num art, last;
    Boolean check_abort;
{
    struct newsgroup *newsgroup = CurrentGroup;
    ...
    for (i = PREFETCH_START_FETCH_STAGE; i <= PREFETCH_LAST_FETCH_STAGE; ) {
      if (setUpGroupIncremental(newsgroup, &i, False, False, False,
				art, last))
	i++;
    ...
    int flags;
{
	...
	/*
	  ...
	  */
	} while (setCurrentArticle(newsgroup, flags & ENTER_UNREAD, 0, 0));
