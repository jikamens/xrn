...
static char XRNrcsid[] = "$Id: internals.c,v 1.132 1995-10-02 18:18:57 jik Exp $";
#endif
...
void initializeNews()
{
    char err_buf[sizeof(CANT_EXPAND_MSG)+MAXPATHLEN];
    ...
    int status;
{
	...
	err_buf = XtMalloc(sizeof(MAYBE_LIST_MSG)+strlen(afterGroup));
	(void) sprintf(err_buf, MAYBE_LIST_MSG, afterGroup);
	if ((ConfirmationBox(TopLevel, err_buf, "yes", "no") == XRN_CB_ABORT) ||
	    (! verifyGroup(afterGroup, &ng))) {
    ...
    int flags;
{
	...
	err_buf = XtMalloc(sizeof(MAYBE_LIST_MSG)+strlen(name));
	(void) sprintf(err_buf, MAYBE_LIST_MSG, name);
	if ((ConfirmationBox(TopLevel, err_buf, "yes", "no") == XRN_CB_ABORT) ||
	    (! verifyGroup(name, &newsgroup))) {
	  XtFree(err_buf);
	  return BAD_GROUP;
	}
      }
