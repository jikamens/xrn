...
static char XRNrcsid[] = "$Id: server.c,v 1.29 1994-11-25 18:57:25 jik Exp $";
#endif
...
authenticate() {
...
#endif
    if (!p)
	sprintf(loginName, "%d", getuid());
    else
	strncpy(loginName, p, sizeof(loginName));
    loginName[sizeof(loginName)-1] = '\0';
  
    strcpy(tmpbuf, "AUTHINFO GENERIC ");
    if ((p = getenv("NNTPAUTH")) || (p = app_resources.authenticator))
	strcat(tmpbuf, p);
    else {
	strcat(tmpbuf, "any ");
	strcat(tmpbuf, loginName);
	builtinauth = 1;
    }
    put_server(tmpbuf);

    cookiefd = fileno(tmpfile());
    ...
    old_env = new_env;
#else
    sprintf(tmpbuf, "%d.%d.%d", fileno(ser_rd_fp), fileno(ser_wr_fp), cookiefd);
