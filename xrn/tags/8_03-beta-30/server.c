...
static char XRNrcsid[] = "$Id: server.c,v 1.136 1997-06-30 02:52:37 jik Exp $";
#endif
 ...
 *
 */
file_cache_file *getarticle(newsgroup, artnumber, retposition, header, rotation,
			    xlation)
...
{
    char command[BUFFER_SIZE], *message, *msg;
    static char *field = NULL;
    static int field_size = 0;
    FILE *articlefp;
    file_cache_file *cache_file_ptr;
    char *ptr;
    ...
    Boolean found_sep = False, is_partial = False, was_partial;
    int real_len, len;
    ...
    cache_file_ptr = (file_cache_file *) XtMalloc(sizeof(*cache_file_ptr));

    if (! (articlefp = file_cache_file_open(FileCache, cache_file_ptr))) {
      sprintf(error_buffer, FILE_CACHE_OPEN_MSG, errmsg(errno));
      FREE(cache_file_ptr);
      ehErrorExitXRN(error_buffer);
    }

    do_chmod(articlefp, file_cache_file_name(FileCache, *cache_file_ptr), 0600);

    for (;;) {
        message = get_data_from_server(!found_sep);

	was_partial = is_partial;

	/* the article is ended by a '.' on a line by itself */
	if (!was_partial && (message[0] == '.') && (message[1] == '\0')) {
	    /* check for a bogus message */
	    if (byteCount == 0) {
		(void) fclose(articlefp);
		file_cache_file_release(FileCache, *cache_file_ptr);
		FREE(cache_file_ptr);
		return(NULL);
	    }
	    break;
	...
	if (*message != '\0') {
	    /*
	      We update the "len" variable throughout the code in this
	      loop, so that we are constantly aware of the current
	      length of the message line, so that we don't have to call
	      strlen at the end of the loop when incrementing our byte
	      count.
	      */
	    len = 0;
	    /* strip backspace characters and count the length of the string */
	    for (ptr = msg = message; *ptr; ptr++, real_len++) {
	      if (*ptr == '\b') {
		if (msg > message) {
		  msg--;
		  len--;
		...
		*msg++ = *ptr;
	    ...
	    is_partial = (real_len >= MAX_BUFFER_SIZE-1);

	    /* strip the headers */
	    if (! found_sep) {
	      if (app_resources.displayLocalTime &&
		  !strncmp(message, "Date: ", 6)) {
		tconvert(message+6, message+6);
		len = strlen(message);
	      }

	      if (header == NORMAL_HEADER) {
		if ((*message == ' ') || (*message == '\t')) { /* continuation line */
		    if (last_stripped)
			continue;
		}
		else {
		    if ((ptr = index(message, ':')) == NIL(char)) {
			continue; /* weird header line, skip */
		    }
		    if (*(ptr+1) == '\0') {
			continue; /* empty field, skip */
		    }
		    if (ptr - message + 1 > field_size) {
		      field_size = ptr - message + 1;
		      field = XtRealloc(field, field_size);
		    }
		    (void) strncpy(field, message, ptr - message);
		    field[ptr - message] = '\0';
		    utDowncase(field);
	    ...
	    if ((xlation == XLATED) && found_sep)
		 ...
		 strlen(message)" line needs to be added after the
		 "utXlate(message)" call. */
		utXlate(message);
#endif /* XLATE */

	    /* handle ^L (poorly?) */
	    if (!was_partial && (*message == '\014')) {
		int i, lines;
	    ...
	    if (fputs(message, articlefp) == EOF) {
		error++;
		break;
	    }
	    byteCount += len;
	...
	if ((fclose(articlefp) == 0) &&
	    file_cache_file_close(FileCache, *cache_file_ptr)) {
	    if (retposition)
		*retposition = position;
        ...
        if (is_partial)
	  (void) get_data_from_server(True);
	(void) fclose(articlefp);
    ...
    (void) sprintf(error_buffer, ERROR_WRITING_FILE_MSG,
		   file_cache_file_name(FileCache, *cache_file_ptr),
		   errmsg(errno));
    file_cache_file_destroy(FileCache, *cache_file_ptr);
    FREE(cache_file_ptr);
    (void) file_cache_free_space(FileCache, 1); /* free up at least one article */
