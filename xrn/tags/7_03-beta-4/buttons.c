...
static char XRNrcsid[] = "$Id: buttons.c,v 1.63 1995-03-30 19:19:32 jik Exp $";
#endif
 ...
 */
{
    ...
    setButtonSensitive("allMove", False);
    setButtonSensitive("ngMove", False);
...
void setButtonSensitive(name, sensitive)
    char *name;
    Boolean sensitive;
{
    Widget w;

    if (! ((w = XtNameToWidget(TopButtonBox, name)) ||
	   (w = XtNameToWidget(BottomButtonBox, name))))
	return;
 ...
 */
{
	...
	TextDisableWordWrap(ArticleText);
    /*     
     * ART -> NG
     *    desensitize bottom box
	...
	setTranslations(NEWSGROUP_MODE);
	TextClear(ArticleText);
	TextEnableWordWrap(ArticleText);
    /*
    ...
    String IndexString;
{
	...
	int ret = moveCursor(FORWARD, IndexString, &top);
	assert(ret);
    }
