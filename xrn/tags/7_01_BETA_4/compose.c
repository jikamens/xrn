...
static char XRNrcsid[] = "$Header: /d/src/cvsroot/xrn/compose.c,v 1.42 1994-11-18 17:18:24 jik Exp $";
#endif
 ...
 */
static char *getUser _ARGUMENTS((void));
...
# endif
	...
	if (! domain) {
	    mesgPane(XRN_SERIOUS, NO_DOMAIN_MSG);
	    freeHeader();
	    return XRN_ERROR;
	}
    ...
    int *msg_size, *msg_total_size;
{
    ...
    (void) sprintf(&message[strlen(message)], "From: %s@%s (%s)\n",
...
#include <maildef.h>
	...
	if (fieldExists("Subject:") == XawTextSearchError) {
		...
		mesgPane(XRN_SERIOUS | XRN_SAME_LINE, EMPTY_ADDED_MSG);
		mesgPane(XRN_SERIOUS | XRN_SAME_LINE, FILL_IN_RESEND_MSG);
	...
	(void) sprintf(newFromString, "From: %s@%s (%s)\n",
		       Header.real_user, Header.sender_host, Header.fullname);
	/*
	...
	if (strcmp(oldFromString, newFromString)) {
	     (void) sprintf(senderString, "Sender: %s@%s (%s)\n",
			    Header.real_user, Header.sender_host,
			    Header.fullname);
	...
	if (fieldExists("Newsgroups:") == XawTextSearchError) {
	    ...
	    addField(buffer);
	    XBell(XtDisplay(TopLevel), 0);
	    mesgPane(XRN_SERIOUS, NO_NEWSGROUPS_MSG);
	    if (*Header.newsgroups)
		mesgPane(XRN_SERIOUS | XRN_SAME_LINE, DEFAULT_ADDED_MSG);
	    else {
		mesgPane(XRN_SERIOUS | XRN_SAME_LINE, EMPTY_ADDED_MSG);
		mesgPane(XRN_SERIOUS | XRN_SAME_LINE, FILL_IN_RESEND_MSG);
	    }
...
/*	stripField("Message-ID:");  .............we need this !!! */
	...
	case POST_FAILED:
	    ...
	    mesgPane(XRN_SERIOUS | XRN_SAME_LINE, SAVING_DEAD_MSG,
...
void cancelArticle()
{
    ...
    *message = '\0';
	...
	if (ErrMessage) {
	    ...
	    mesgPane(XRN_SERIOUS | XRN_SAME_LINE, "  ");
	    mesgPane(XRN_SERIOUS | XRN_SAME_LINE, CANCEL_ABORTED_MSG);
	    FREE(ErrMessage);
	}
	else {
