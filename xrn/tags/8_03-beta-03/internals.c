...
static char XRNrcsid[] = "$Id: internals.c,v 1.152 1996-06-06 08:19:09 jik Exp $";
#endif
    ...
    int max;
{
    ...
    char *subject, *author, *newsgroups, *subj WALL(= 0);
    ...
    Boolean update_last, kill_files, unread_only;
{
	...
	if (! EMPTY_GROUP(newsgroup)) {
		...
		finished = getlineslist(newsgroup, newsgroup->current,
					...
					(stage && ! FinishingPrefetch)
					? chunk_size : 0);
	    if (newsgroup->fetch_newsgroups && (! stage || *stage == 6))
	      finished = getnewsgroupslist(newsgroup, newsgroup->current,
					   newsgroup->last, unread_only,
					   (stage && ! FinishingPrefetch)
					   ? chunk_size : 0);
    ...
    art_num artNum;
{
    ...
    if (artNum <= 0) {
      num_width = digits(newsgroup->last);
