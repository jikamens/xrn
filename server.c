...
static char XRNrcsid[] = "$Id: server.c,v 1.130 1997-04-07 00:34:11 jik Exp $";
#endif
 ...
 */
static char *xhdr_single _ARGUMENTS((struct newsgroup *, char *));

static char *xhdr_single(newsgroup, buffer)
    struct newsgroup *newsgroup;
    char *buffer;
{
    char *message, *ptr;
    ...
    char **string;
{
    ...
    *string = xhdr_single(newsgroup, buffer);
    return;
}
#endif
...
char *xhdr_id(newsgroup, id, field)
     struct newsgroup *newsgroup;
     char *id;
     char *field;
{
  ...
  return xhdr_single(newsgroup, buffer);
}
